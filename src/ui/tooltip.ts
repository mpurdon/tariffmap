...
export function showTooltip(info: PickingInfo, ds: Dataset) {
  ...
  if (info.layer?.id === 'flow-arcs' || info.layer?.id === 'base-arcs') {
    ...
    const rows = a.active
      ...
      .join('');
    html = `<div class="tt-head"><span class="swatch" style="background:${imposerCss(a.imposer)}"></span>${from.name} <span class="arrow">→</span> ${to.name}</div>
    ...
    html = `<div class="tt-head">${n.name}<span class="dim"> · ${n.capital}</span></div><div class="dim">Click to focus</div>`;
  } else if (info.layer?.id === 'countries') {
    const ent = ds.entityByNum.get(String((info.object as {id: string}).id));
    html = `<div class="tt-head">${ent?.name ?? (info.object as {properties: {name: string}}).properties.name}</div><div class="dim">${ent ? 'Click to focus' : 'No tracked tariffs'}</div>`;
  }
